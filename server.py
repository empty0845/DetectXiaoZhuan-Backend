...
sqldb = mysql.mysql()
# 初始化模型
model = Yolo.Yolov5ONNX("./models/xiaozhuan_model2.onnx")
# 读取模型推理出结果ID -> UUID，name
id_to_uuid = {}
with open("id_to_uuid.json", "r", encoding="utf-8") as f:
    id_to_uuid = f.read()
...
def get_config():
    ret = {
        "data": {
            "imageUrl": random.choice(["/static/new_cover/" + '1' + ".png"]),
            # "color": [
            #     "red",
            #     "blue",
            #     "yellow",
...
def uploadImage():
    ...
    outbox = Yolo.filter_box(output, 0.8, 0.5)
